...
async function importListings() {
  if (
    ...
    throw new Error(
      'Legacy importer is limited to rentalk_local. Production requires a validated, authorized feed.',
    );
  }
  ...
  const csvPaths = [
    path.resolve(
      __dirname,
      '../../nhatot-crawler/data/nhatot_100k_combined.csv',
    ),
  ...
  for (const csvPath of csvPaths) {
    ...
    for await (const line of rl) {
      ...
      const sourceUrl =
        ...
        (getVal('post_url')
          ? getVal('post_url')
          : `https://www.nhatot.com/${listId}.htm`);
      ...
      const priceInMillion =
        rawPrice > 1000 ? (rawPrice / 1000000).toFixed(2) : rawPrice.toFixed(2);
      if (priceInMillion <= 0 || priceInMillion > 500) continue;

      const area =
        parseFloat(getVal('size')) || parseFloat(getVal('area')) || 30;
      const bedrooms =
        parseInt(getVal('rooms')) || parseInt(getVal('bedrooms')) || 1;
      const bathrooms =
        parseInt(getVal('toilets')) || parseInt(getVal('bathrooms')) || 1;

      let lat =
        parseFloat(getVal('latitude')) ||
        parseFloat(getVal('location_latitude'));
      let lng =
        parseFloat(getVal('longitude')) ||
        parseFloat(getVal('location_longitude'));
      if (
        !Number.isFinite(lat) ||
        !Number.isFinite(lng) ||
        lat < -85 ||
        ...
        // Missing locations must never be invented or used for routing.
        lat = null;
        lng = null;
      }

      const province =
        getVal('region_name') || getVal('province') || 'Ho Chi Minh City';
      const district = getVal('area_name') || getVal('district') || 'Quận 1';
      const ward = getVal('ward_name') || getVal('ward') || '';
      const displayedAddress = ward
      ...
      if (
        cat === '1010' ||
        name.toLowerCase().includes('căn hộ') ||
        name.toLowerCase().includes('chung cư')
      )
        pType = 'apartment';
      else if (
        cat === '1030' ||
        name.toLowerCase().includes('nhà phố') ||
        name.toLowerCase().includes('biệt thự') ||
      ...
      if (totalImported % 50 === 0) {
        console.log(
          `  -> Imported: ${totalImported} listings into MySQL rental_posts`,
        );
      }
